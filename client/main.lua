        ...
        options[#options + 1] = {
            ...
            onSelect = function ()
                ...
                if alert == 'confirm' then
                    TriggerServerEvent('hw_vending:buyVending', point.label)
                end
            end
        }
    end
        ...
        if point.owner == GetIdentifier() then
            options[#options+1] = {
                ...
                onSelect = function()
                    ...
                    if alert == 'confirm' then
                        TriggerServerEvent('hw_vending:sellVending', point.label)
                    end
                end
            }
...
function SetupVendings()
   ...
   local data = lib.callback.await('hw_vending:fetchVendings', false)
...
RegisterNetEvent('hw_vending:sync', function(data, clear)
...
RegisterNetEvent('hw_vending:startCreating', function(players)
    if source == '' then return end
    local vending = {}
    ...
    elseif input[4] == 'b' then
        local jobs = lib.callback.await('hw_vending:getJobs', 100)
        ...
        if not owner[1] then
            ...
            local grades = lib.callback.await('hw_vending:getGrades', 100, owner[1])
    ...
    CreateThread(function ()
        while true do
            ...
            if IsDisabledControlPressed(0, 176) then
                ...
                DeleteObject(obj)
                TriggerServerEvent('hw_vending:createVending', vending)
...
RegisterNetEvent('hw_vending:client:dellvending', function(data)
    if source == '' then return end
    ...
    TriggerServerEvent('hw_vending:server:dellvending', input[1])
end)


RegisterNetEvent('hw_vending:selectCurrency', function(payload)
    if source == '' then return end
    local itemNames = {}
    ...
    TriggerServerEvent('hw_vending:setData', input[1], input[2], payload)
end)

lib.callback.register('hw_vending:choseVending', function(options)
