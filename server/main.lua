...
RegisterNetEvent('hw_vending:setData', function(price, currency, payload)
...
RegisterNetEvent('hw_vending:SetUpStore', function(store)
    Wait(300)
...
local function SetUpHooks(inventoryFilter)
    ...
    swapHook = exports.ox_inventory:registerHook('swapItems', function(payload)
        if payload.toType == 'stash' then
            ...
            TriggerClientEvent('hw_vending:selectCurrency', payload.source, payload)
        else
            TriggerEvent('hw_vending:SetUpStore', payload.fromInventory)
        end
...
MySQL.ready(function()
    ...
    local success, error = pcall(MySQL.scalar.await, 'SELECT 1 FROM `hw_vending`')

    if not success then
        MySQL.query([[
            CREATE TABLE IF NOT EXISTS `hw_vending` (
    ...
    local result = MySQL.query.await('SELECT * FROM `hw_vending`')
    ...
    if result[1] then
        ...
        for k,v in pairs(result) do
            ...
            TriggerEvent('hw_vending:SetUpStore', v.name)
        end
...
lib.callback.register('hw_vending:fetchVendings', function(source)
    return Vending
end)
...
lib.addCommand('addvending', {
    ...
    TriggerClientEvent('hw_vending:startCreating', source, options)
end)
...
lib.addCommand('dellvending', {
    ...
    TriggerClientEvent('hw_vending:client:dellvending', source, options)
end)
...
lib.addCommand('findvending', {
    ...
    local cb = lib.callback.await('hw_vending:choseVending', source, options)
...
RegisterNetEvent('hw_vending:server:dellvending', function(shop)
    if Vending[shop] then
        MySQL.query('DELETE FROM `hw_vending` WHERE `name` = ?', { shop })

        exports.ox_inventory:ClearInventory(('stash-money-%s'):format(shop))
        exports.ox_inventory:ClearInventory(('%s'):format(shop))

        Vending[shop] = nil
        TriggerClientEvent('hw_vending:sync', -1, Vending, true)
    end
end)

RegisterNetEvent('hw_vending:buyVending', function(name)
    local src = source
    ...
    if Vending[name] then
        if exports.ox_inventory:Search(src, 'count', 'money') >= Vending[name].price then
            ...
            MySQL.update('UPDATE `hw_vending` SET `data` = ? WHERE `name` = ?', {json.encode(Vending[name], {sort_keys = true}), name})
            TriggerClientEvent('hw_vending:sync', -1, Vending, true)
...
RegisterNetEvent('hw_vending:sellVending', function(name)
    local src = source
...
RegisterNetEvent('hw_vending:createVending', function(data)
    local src = source

    MySQL.insert('INSERT INTO `hw_vending` (name, data) VALUES (?, ?)', {data.name, json.encode(data, {sort_keys = true})})
    ...
    TriggerEvent('hw_vending:SetUpStore', data.name)

    SetUpHooks(inventoryFilter)

    TriggerClientEvent('hw_vending:sync', -1, Vending, false)
end)

lib.callback.register('hw_vending:getJobs', function(source)
    local jobs = GetJobs()
    local options = {}
...
lib.callback.register('hw_vending:getGrades', function(source, job)
    local jobs = GetJobs()
    local options = {}
...
local function saveDB()
    ...
    for k,v in pairs(Vending) do
        insertTable[#insertTable + 1] = { query = 'UPDATE `hw_vending` SET `data` = ? WHERE `name` = ?', values = { json.encode(v, {sort_keys = true} ), v.name } }
    end
